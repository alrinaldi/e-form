...
" id="sidebar">
  ...
  <div class="modal-dialog modal-dialog modal-dialog-centered modal-xl " >
    <div class="modal-content">
      ...
      <h2>Master Size <span id="target13" style="display: none"></span></h2>
      <form action="">
  ...
  <script>
    ...
    $('.check_all').click(function() {
        ...
        function tampil_approval() {
				$.ajax({
					type: 'ajax',
					url:'<?php echo site_url('admin/c_uploaded/size_complete')?>',
					async: false,
					dataType: 'json',
					success: function (data) {
						...
						for (i = 0; i < data.length; i++) {
                            html += '<tr id="' + data[i].id_m + '">' +
                                '<td>'+
                                '<input type="checkbox" name="id_master_form[]" class="form-control chk_boxes1" value="'+data[i].id_m+'" style="width:20px"/>'+
                                '</td>'+
								'<td>' + data[i].id_m + '</td>' +
								'<td>' + data[i].nama + '</td>' +
                                 '<td>'+
                                '<button class="btn btn-primary" data-toggle="modal" data-target="#staticBackdrop" data-id="'+data[i].id_m+'" ><span class="fa fa-edit"></span></button>'+
                                '</td>' +
                                '<td>'+
                                '<a class="btn btn-primary" href="<?php echo base_url();?>admin/c_export/size_export/'+data[i].id_m+'" target="_blank"><span class="fa fa-file"></span></a>'+
                                '</td>' +
            ...
            $('#btn_size').on('click',function(){
                ...
                $.ajax({
                    type: "POST",
                    url : '<?php echo site_url('admin/c_uploaded/detail_size')?>',
                    dataType:'json',
                    data:{id_master_form:id_master_form},
                    success:function(data){
                        var tablesize='';
                        for(i = 0; i < data.length; i++ ){
                            ...
                            $.ajax({
                                type: "POST",
                                url : '<?php echo site_url('admin/c_uploaded/get_size')?>',
                                dataType:'json',
                                 data:{idi:idi},
