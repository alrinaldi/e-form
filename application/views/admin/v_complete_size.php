...
" id="sidebar">
      ...
      <div class="main-panel">
        ...
        <button class="btn btn-primary" id="submit_item">List Item Complete</button>
        <p class="pt-3"></p>
...
<div class="modal fade " id="Complt" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog modal-dialog-centered  " >
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">Size Complete </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <form action="">
       <input type="hidden" name="idmi" id="idmi">
       <h4 id="paraf"></h4>
        <p class="pt-3" > Size has been Uploaded?</p>
      </div>
    </form>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
        <button type="button" class="btn btn-primary" id="btn_com">Yes</button>
      </div>
    </div>
  </div>
</div>
  ...
  <script>
    ...
    $('.check_all').click(function() {
        ...
        function tampil_approval() {
				$.ajax({
					type: 'ajax',
					url:'<?php echo site_url('admin/c_complete/size_complete')?>',
					async: false,
					dataType: 'json',
					success: function (data) {
						...
						for (i = 0; i < data.length; i++) {
                            html += '<tr id="' + data[i].id_master_form + '">' +
								...
								'<td>' + data[i].nama + '</td>' +
                                 '<td>'+
                                '<button class="btn btn-primary" data-toggle="modal" data-target="#staticBackdrop" data-id="'+data[i].id_master_form+'" ><span class="fa fa-edit"></span></button>'+
                                '</td>' +
                                '<td>'+
                                '<a class="btn btn-primary" href="<?php echo base_url();?>admin/c_export/size_export/'+data[i].id_master_form+'" target="_blank"><span class="fa fa-file"></span></a>'+
                                '</td>' +
                                '<td>'+
                                '<button class="btn btn-primary" data-toggle="modal" data-target="#Complt" data-id="'+data[i].id_master_form+'"><span class="fa fa-edit"></span></a>'+
                                '</td>' +
                                '</tr>'
            ...
            $('#btn_size').on('click',function(){
                ...
                $('#Complt').on('show.bs.modal',function(event){
                    ...
                    $('#paraf').html(id_master_form);
                    $('#btn_com').on('click',function(){
                    $.ajax({
                    type:'POST',
                    url: '<?php echo site_url('admin/c_complete/uploaded_size')?>',
                    dataType:'JSON',
                    data:{id_master_form:id_master_form},
                    success:function(data){
                        $('[name="idmi"]').val("");
                ...
                $.ajax({
                    type: "POST",
                    url : '<?php echo site_url('admin/c_complete/detail_size')?>',
                    dataType:'json',
                    data:{id_master_form:id_master_form},
                    success:function(data){
                        var tablesize='';
                        for(i = 0; i < data.length; i++ ){
                            ...
                            $.ajax({
                                type: "POST",
                                url : '<?php echo site_url('admin/c_complete/get_size')?>',
                                dataType:'json',
                                 data:{idi:idi},
