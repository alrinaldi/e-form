  ...
  <div class="modal-dialog modal-dialog modal-dialog-centered  " style="max-width: 100%;">
    <div class="modal-content">
      ...
      <form action="">
        ...
        $cc = $this->session->userdata('wct');
        $cekappr = $this->db->query("SELECT * FROM workflow_item where level = '$fn' and flow_name = '$cc'");
        $cappr = $cekappr->num_rows();
        if($cappr>0){
        ?>
        <button class="btn btn-primary" id="submit_item">Approve</button>
        <?php
        }else{
       ?>
        <button class="btn btn-primary" id="submit_item" disabled>Approve</button>
        <?php
        }
        ?>
        <p class="pt-3"></p>
...
<div class="modal fade " id="Complt" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog modal-dialog-centered  " >
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">Item Complete </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <form action="">
       <input type="hidden" name="idmi" id="idmi">
        <p class="pt-3"> Item has been Uploaded?</p>
      </div>
    </form>
      <div class="modal-footer">
  ...
  <script>
    ...
    $('.check_all').click(function() {
        ...
        function tampil_approval() {
				$.ajax({
					type: 'ajax',
					url:'<?php echo site_url('admin/c_complete/item_complete')?>',
					async: false,
					dataType: 'json',
						...
						for (i = 0; i < data.length; i++) {
                            html += '<tr id="' + data[i].id_master_form + '">' +
								...
								'<td>' + data[i].requestor + '</td>' +
                                 '<td>'+
                                '<a class="btn btn-primary" href="<?php echo base_url();?>admin/c_item_it/detail_form/'+data[i].id_master_form+'" target="_blank"><span class="fa fa-edit"></span></a>'+
                                '</td>' +
                                '<td>'+
                                ...
                                '</td>' +
                                '<td>'+
                                '<button class="btn btn-primary" data-toggle="modal" data-target="#Complt" data-id="'+data[i].id_master_form+'" ><span class="fa fa-edit"></span></button>'+
                                '</td>' +
