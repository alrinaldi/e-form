		...
		<div class="modal-dialog  modal-dialog-centered  modal-xl " style="max-width:100%">
			<div class="modal-content">
				...
				<div class="modal-body">
                    ...
                    <p class="pt-4"></p>
                   <table id="table1231" class="table table-bordered table-hover">
                   <thead style="background-color:orange !important;color:black" class="text-center font-weight-bold">
                        <tr>

                            <th>User Section Head</th>
                            <th>User Department Head</th>
                            <th>User Division Head</th>
                            <th>IT Business Process</th>
                            <th style="width:19%">Information Technology Department Head</th>
                        </tr>
                            </thead>
                        <tbody id="" class="text-center">
                            ...
                            $cap = $this->db->query("SELECT * FROM workflow_form where type = 'Master Size'");
                            $ccap = $cap->num_rows();
                            
                            $ap = $this->db->query("SELECT a.*,DATE_FORMAT(a.tanggal, '%Y-%m-%d') as date,DATE_FORMAT(a.tanggal, '%H:%i') as time,b.nama 
                             FROM approval_master a join pegawai b on a.approval = b.nrp where a.id_master_form = '$id_vend' and a.status like '%Approval%' and
                             a.type = 'Master Size'  order by a.tanggal ");
                            $k=0;
