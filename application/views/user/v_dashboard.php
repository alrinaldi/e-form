...
<?php
$nrpc = $this->session->userdata('nrp');
$deptc = $this->session->userdata('dept');
$divs = $this->session->userdata('divisi');
if($this->session->userdata('level')!='Division Head' && $divs != 'FINANCE ACCOUNTING & IT DIVISION'  ){
  ...
  on a.id_master_form = b.id_master_form join pegawai c on b.requestor = c.nrp where c.dept = '$deptc' group by MONTHNAME(b.created_date) order by MONTH(b.created_date) ");
  ...
  endforeach;
}else if ($this->session->userdata('level')=='Division Head' && $deptc != 'FINANCE ACCOUNTING & IT DIVISION'){
  $crs = $this->db->query("SELECT count(a.item_id) as jmlh,MONTHNAME(b.created_date) as bulan from master_item a join approval_master_item b
  on a.id_master_form = b.id_master_form join pegawai c on b.requestor = c.nrp where c.divisi = '$deptc' group by MONTHNAME(b.created_date) order by MONTH(b.created_date) ");
  ...
  foreach($crs->result() as $ct):
   ...
   // $hasilp[] = "["."'".$kategorip."'".",".$percentsp."]";
  endforeach;
}else if($this->session->userdata('level')!='Division Head' && $deptc == 'ACCOUNTING & TAX'  ){
  $crs = $this->db->query("SELECT count(a.item_id) as jmlh,MONTHNAME(b.created_date) as bulan from master_item a join approval_master_item b
...
else if($this->session->userdata('level')!='Division Head' && $deptc == 'FINANCIAL PLANNING & ANALYSIS'){
  $crs = $this->db->query("SELECT count(a.item_id) as jmlh,MONTHNAME(b.created_date) as bulan from master_item a join approval_master_item b
  on a.id_master_form = b.id_master_form  group by MONTHNAME(b.created_date) order by MONTH(b.created_date) ");
...
if($this->session->userdata('level')!='Operator' && $this->session->userdata('dept')=='ACCOUNTING & TAX' || $deptc=='FINANCIAL PLANNING & ANALYSIS'){
...
$chrsize = $this->db->query("SELECT count(b.size) as jmlh,MONTHNAME(a.created_date) as bulan from master_size a join
 size_list b on a.id = b.id_master where a.requestor = '$nrpc' group by MONTHNAME(a.created_date) order by MONTH(a.created_date)
");
foreach($chrsize->result() as $cs):
$jmlhs[] = (int)$cs->jmlh;
$blns[] = $cs->bulan;
endforeach;
}else if($this->session->userdata('level')=='Section Head' || $this->session->userdata('level')=='Department Head' ){
  $chrsize = $this->db->query("SELECT count(b.size) as jmlh,MONTHNAME(a.created_date) as bulan from master_size a join
 size_list b on a.id = b.id_master join pegawai  c on a.requestor = c.nrp  where c.dept = '$deptc' group by MONTHNAME(a.created_date) order by MONTH(a.created_date)
");
foreach($chrsize->result() as $cs):
$jmlhs[] = (int)$cs->jmlh;
$blns[] = $cs->bulan;
endforeach;
  ...
  $chrsize = $this->db->query("SELECT count(b.size) as jmlh,MONTHNAME(a.created_date) as bulan from master_size a join
  size_list b on a.id = b.id_master join pegawai  c on a.requestor = c.nrp  where c.divisi = '$deptc' group by MONTHNAME(a.created_date) order by MONTH(a.created_date)
 ");
 foreach($chrsize->result() as $cs):
 $jmlhs[] = (int)$cs->jmlh;
 $blns[] = $cs->bulan;
