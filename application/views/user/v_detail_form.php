...
" id="sidebar">
  ...
  <div class="card-body">
      ...
      $ccekfix = $cekfix->num_rows();
      if($ccekitem > 0 &&  $ccekfix > 0 && $this->session->userdata('level')=='Section Head'){
      ?>
        <button type="submit" class="btn btn-primary" id="submit_apprl" disabled>Approve</button>           
      <?php
      }else{
      ?>
      <form action="<?php echo base_url('user/c_acc_item/submit_item')?>" method="POST">
...
<p class="pt-3"></p>
    ...
    <thead style="background-color:orange !important;color:black" class="text-center font-weight-bold">
                ...
                <tr>
                    ...
                    <label>project</label>
                    <input type="text" class="form-control fixasst" id="<?php echo $i['item_id']?>" value="<?php echo $i['fixed_asset_group']?>"  placeholder="Title" style="width:200px;">
                  </div>
                     <?php
                        }else{
                     ?>
                    ...
                    <?php if($this->session->userdata('level')=='Section Head'){?>
                    <td>
                    <a class="btn btn-primary" data-toggle="modal" data-target="#ModalGroup<?php echo $i['item_id'];?>"><span class="fa fa-pencil"></span></a>
                    </td>
                    <?php }else{
                      }?>
...
<td><?php echo $i['purchase_unit']?></td>
  ...
  <div class="modal-dialog modal-dialog modal-dialog-centered  ">
    <div class="modal-content">
      ...
      <form action="<?php echo base_url('user/c_acc_item/edit_group')?>" method="post">
      <div class="modal-body">
      <h2>Item Group <span id="target13" style="display: none"></span></h2>
...
?>
<option value="<?php echo $g['ITEMGROUPID']?>"><?php echo $g['ITEMGROUPID']?></option>
<?php
endforeach;
?>
</select>         
...
//          console.log(val);
    ...
    $("#"+id).keypress(function(e){
      ...
      if(e.which ==13){
        ...
        $.ajax({
          type:"POST",
          url:'<?php echo site_url('user/c_acc_item/addprj')?>',
          dataType:"JSON",
          data:{id:id,val:val},
